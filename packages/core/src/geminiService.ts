...
export const generateStickerImage = async (
  request: StickerRequest,
  allStyles: StickerStyle[] = STICKER_STYLES,
): Promise<GeneratedImage> => {
  const [image] = await generateStickers({ ...request, quantity: 1 }, allStyles);
  return image;
};
...
export const generateStyleDescription = analyzeStyleFromImage;
...
export const generatePromptSuggestions = generateRelatedPrompts;
