...
type APISettingsResolver = () => APISettings;

let apiSettingsResolver: APISettingsResolver | undefined;
...
export const configureAPISettingsResolver = (resolver: APISettingsResolver | undefined) => {
  apiSettingsResolver = resolver;
};

export const loadAPISettings = (): APISettings => {
  if (!apiSettingsResolver) {
    return getDefaultAPISettings();
  }

  return normalizeAPISettings(apiSettingsResolver());
};
